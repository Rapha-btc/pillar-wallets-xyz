// fakfun-wallet-v16-faktory.test.ts -- the 8 faktory-* functions.
//
// Run: npx vitest run tests/fakfun-wallet-v16-faktory.test.ts -- --manifest tests/cl-v16/Clarinet.toml
//
...
describe("faktory-burn-bob", () => {
  it("runs on the admin path and on the passkey path", () => {
    funded();
    const r = simnet.callPublicFn(WALLET, "faktory-burn-bob", [Cl.none(), Cl.none()], OWNER);
    expect(Cl.prettyPrint(r.result), "reaches burn-bob-faktory").toMatch(/^\(ok |^\(err u\d+\)/);
    expect(simnet.callPublicFn(WALLET, "faktory-burn-bob",
      [Cl.none(), Cl.none()], RANDOM).result).toBeErr(Cl.uint(E.UNAUTH));
  });
});
...
describe("wager-deposit: the success path", () => {
  ...
  it("whitelists the token, registers the wallet, then deposits", () => {
    ...
    expect(simnet.callPublicFn(WAGER, "register-wallet",
      [Cl.contractPrincipal(WD, "fakfun-wallet-v16"),
    ...
    expect(simnet.callReadOnlyFn(WAGER, "get-registered-wallet", [pubkeyCV], OWNER).result)
      .toBeSome(Cl.contractPrincipal(WD, "fakfun-wallet-v16"));
...
describe("faktory: the last branches line coverage flagged", () => {
  ...
  it("faktory-burn-bob on the PASSKEY path, with and without a station", () => {
    funded();
    // no station: takes the (match gas ... true) else-arm
    const r1 = simnet.callPublicFn(WALLET, "faktory-burn-bob",
      [Cl.some(sign(topic("faktory-burn-bob", { "auth-id": Cl.uint(next()) }), id)),
       Cl.none()], RELAYER);
    expect(Cl.prettyPrint(r1.result)).toMatch(/^\(ok |^\(err u\d+\)/);
    // with a station
    const g0 = gasCounter();
    const r2 = simnet.callPublicFn(WALLET, "faktory-burn-bob",
      [Cl.some(sign(topic("faktory-burn-bob", { "auth-id": Cl.uint(next()) }), id)),
       Cl.some(stationCV)], RELAYER);
    if (Cl.prettyPrint(r2.result).startsWith("(ok")) {
      expect(gasCounter() - g0).toBe(BigInt(GAS_FEE));
    }
  });

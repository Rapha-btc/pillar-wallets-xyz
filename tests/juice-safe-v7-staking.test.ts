// juice-safe-v7-staking.test.ts -- pox-5 staking IS testable in simnet.
//
// Run: npx vitest run tests/juice-safe-v7-staking.test.ts -- --manifest tests/cl-v7/Clarinet.toml
//
...
describe("v6 rewards: the full payout chain, sBTC -> pox-5 -> signer -> staker", () => {
  ...
  it("pays the staked safe its sBTC share, and refuses to pay twice", () => {
    ...
    expect(Cl.prettyPrint(simnet.callPublicFn(SIGNER, "pay-stx-stakers",
      [Cl.list([Cl.contractPrincipal(WD, "juice-safe-v7")]), Cl.uint(cycle), Cl.uint(0)],

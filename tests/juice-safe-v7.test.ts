// juice-safe-v7.test.ts -- Clarinet scenario tests against the DEPLOYED source.
//
// Run:  npx vitest run tests/juice-safe-v7.test.ts -- --manifest tests/cl-v7/Clarinet.toml
//
// The contract compiled here is byte-identical to
// SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.juice-safe-v7 on mainnet, and every
...
const domainHash = () =>
  cvHash(
    Cl.tuple({
      ...
      wallet: Cl.contractPrincipal(WD, "juice-safe-v7"),
    }),
  );
...
describe("juice-safe-v7: deploys and initialises against real mainnet dependencies", () => {
...
describe("juice-safe-v7: onboard guards", () => {
  it("refuses the contract itself as recovery", () => {
    expect(verifyContract().result).toBeOk(Cl.bool(true));
    const r = simnet.callPublicFn(
      WALLET, "onboard",
      [pubkeyCV, Cl.principal(OWNER), Cl.contractPrincipal(WD, "juice-safe-v7"),
       Cl.uint(STX_THRESHOLD), Cl.uint(SBTC_THRESHOLD), Cl.uint(MIN_COOLDOWN)],
      FAKFUN_DEPLOYER,
    );
    expect(r.result).toBeErr(Cl.uint(ERR_UNAUTH));
...
describe("juice-safe-v7: config change needs two different factors", () => {
...
describe("juice-safe-v7: max-gas raise needs two different factors", () => {
...
describe("juice-safe-v7: thresholds queue rather than move", () => {
...
describe("juice-safe-v7: the contract is never its own admin", () => {
  it("is-admin-calling rejects the contract's own principal", () => {
    onboarded();
    expect(simnet.callReadOnlyFn(WALLET, "is-admin-calling",
      [Cl.contractPrincipal(WD, "juice-safe-v7")], OWNER).result)
      .toBeErr(Cl.uint(ERR_UNAUTH));
  ...
  it("recover-inactive-wallet refuses to seat the contract", () => {
    ...
    expect(simnet.callPublicFn(WALLET, "recover-inactive-wallet",
      [Cl.contractPrincipal(WD, "juice-safe-v7")], RECOVERY).result)
      .toBeErr(Cl.uint(ERR_UNAUTH));
  });
});

describe("juice-safe-v7: inactivity recovery", () => {

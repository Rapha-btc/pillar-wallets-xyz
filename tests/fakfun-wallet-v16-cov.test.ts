// fakfun-wallet-v16-cov.test.ts -- what MEASURED line coverage exposed in v16.
//
// Run: npx vitest run tests/fakfun-wallet-v16-cov.test.ts -- --manifest tests/cl-v16/Clarinet.toml
//
...
import {
  ...
  seated, deployV16, fundSTX, fundSBTC, sign, topic, pubkeyCV, FAKFUN_DEPLOYER,
...
describe("v16 cov: inactivity recovery actually recovering", () => {
  ...
  it("BY DESIGN: until recovery is designated, an inactive wallet has no recovery path", () => {
    ...
    // (fakfun-wallet-v16.clar:1982), and v16's recovery-address DEFAULTS to the burn
  ...
  it("with recovery designated, it seats a new admin and resets the clock", () => {
    ...
    expect(simnet.callPublicFn(WALLET, "recover-inactive-wallet",
      [Cl.contractPrincipal(WD, "fakfun-wallet-v16")], RANDOM).result)
      .toBeErr(Cl.uint(E.UNAUTH));

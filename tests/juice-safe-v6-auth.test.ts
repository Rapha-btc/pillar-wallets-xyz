// juice-safe-v6-auth.test.ts -- the last four error branches in v6.
//
// Run: npx vitest run tests/juice-safe-v6-auth.test.ts -- --manifest tests/cl-v6/Clarinet.toml
//
// A code-by-code audit of juice-safe-v6.clar found 20 distinct error codes across 79
...
describe("v6 auth: an op the passkey created cannot be fast-tracked by the passkey", () => {
  ...
  //   juice-safe-v6.clar:694  (asserts! (not (get passkey-created op)) err-forbidden)
  let id = 300;
  ...
  it("an admin cannot propose ITSELF as the next admin", () => {
    //   juice-safe-v6.clar:1117  (asserts! (not (is-eq new-admin tx-sender)) err-forbidden)
    onboarded();
...
describe("v6 auth: a captured signature cannot be replayed", () => {
  // consume-signature is reached from exactly one place (juice-safe-v6.clar:573), the

...
const FIXED_NAME = "zz-v9-fixed";
const FIXED = `${DEPLOYER}.${FIXED_NAME}`;      // corrected copy, deployed in-sim
...
async function main() {
  ...
  const v9src = fs.readFileSync("contracts/fakfun-wallet-v9.clar", "utf8");
  const fixedSrc = v9src.replace(
    `'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-wallet-v8\n      )`,
    `'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.${FIXED_NAME}\n      )`);
  if (fixedSrc === v9src) throw new Error("patch did not apply - register-wallet text moved");
  ...
  b.withSender(DEPLOYER).addContractDeploy({
    contract_name: FIXED_NAME, source_code: fixedSrc,
    clarity_version: ClarityVersion.Clarity5,
  });
  plan.push({ kind: "deploy", label: `B0 deploy ${FIXED_NAME} (register-wallet -> itself)` });
  call("B1 set-verified-contract(zz-v9-fixed)", DEPLOYER, WALLET_CORE,
  ...
  console.log("=== fakfun-wallet-v9 - stxer harness (v8 -> v9 delta only) ===\n");

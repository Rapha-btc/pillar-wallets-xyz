;; juice-safe-v5: juice-safe-v4 with the config surface moved behind the passkey.
;;
;; SIX CHANGES. Everything else is juice-safe-v4 verbatim.
;;
...
;;
;; 5. RECOVERY IS MANDATORY AT ONBOARD. Was (optional principal). Onboarding with
;;    none meant a lost admin key lost the funds outright, permanently -- the one
;;    unrecoverable vector in v4, and it was chosen silently at onboard time.
;;    Now required, and rejected if it is the burn address, this contract, or the
;;    owner. ONBOARD'S SIGNATURE CHANGED: callers must pass a bare principal, not
;;    an optional.
;;
...
;;
;; juice-safe-v4: juice-safe-v2 plus a metered, fused gas channel.
;;
;; v2 left one sBTC outflow uncounted. A gasless call pays a caller-supplied
...
;; user signs, forever, against no cap. See the block on spent-this-period and
;; pay-gas-accounted below for the full reasoning, and README-GAS-METERING.md
;; for why the obvious fixes (charge it to sbtc-threshold; block on it) were
;; both tried and both rejected.
;;
;; Everything else is v2 verbatim. Only register-wallet's argument changes, to
;; name this contract -- see the NOTE at onboard.
;;
;; juice-safe-v1: juice-safe-v0 with the unstake allowance fixed. v0's unstake
;; declared (with-staking (locked-ustx)) and could NEVER succeed -- (err u128)
...
(define-read-only (verify-signature
  ...
  (let ((auth-rp-id (unwrap!
    ...
    ;; juiceofbtc.com ONLY. v0/v1 inherited a five-origin whitelist from
    ;; jing-mm-safe (pillarwallets.xyz, jingswap.com, fak.fun, fakfun.com); a
    ;; Juice safe has no reason to accept a signature minted on any of them, so
    ;; the surface is narrowed to the single origin that actually serves it.
...
(define-public (onboard
  ...
  (begin
    ...
    ;; RECOVERY IS MANDATORY. In v4 this was (optional principal), and onboarding
    ;; with none meant losing the admin key lost the funds outright -- the one
    ;; genuinely permanent loss vector in the contract, chosen at onboard by
    ;; whoever called it. It cannot be none here, cannot be this contract, and
    ;; cannot be the owner, since a recovery address equal to the owner recovers
    ...
    (try! (as-contract? ()
      (try! (contract-call?
        'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-wallet-core
        register-wallet
        'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.juice-safe-v5
      ))
    ))
    (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-wallet-core
